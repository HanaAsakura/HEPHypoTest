...
void generatePseudoData(string fileName="./bkgplussignal.root",
                        string  histName="transverse_mass_lumi",
                        int Nevents= 100,
   ...
   // Import the histogram from which the pseudodata will be generated
    ...
    TH1* h_pseudoData = (TH1*)h_pseuDS->Clone();

	//Get the number of bins for the loop
	TAxis * axis = h_pseudoData->GetXaxis();
   ...
   */ 
	...
	n_bini_i = h_pseuDS->GetBinContent(i) ;
        ...
        if(n_bini_i!=0){
			...
			//w.factory("Gaussian::p(x[0,5.0*n_bini_i],mu[n_bini_i],sigma[sqrt(n_bini_i)])");
			//w.Print();
//this method creates the PDF within the object "w", according to the string defined in the previous line. This creates the PDF
			first = 0;
            }

		else{
       		     //Change the parameters of the previously created Gaussian PDF with the content of the current bin
			w.var("x")->setMax(5.0*n_bini_i);            	   	
			//w::x.setMax(5.0*n_bini_i) ;
			
			w.var("mu")->setVal(n_bini_i);            		
			//w::mu.setVal(n_bini_i);
			
			w.var("sigma")->setVal(sqrt(n_bini_i));            				//w::sigma.setVal(sqrt(n_bini_i));
            		RooAbsPdf* model = w.pdf("p") ;
			//RooDataSet* obsData = w::p.generate(w::x,Nevents) ;
			RooDataSet* obsData = model->generate(*w.var("x"),Nevents);
         		//Calculate mean of data generated
            		mean = obsData->mean(*w.var("x")) ;
            		mean_i = TMath::Nint(mean) ;
            
            		//Calculate the standard deviation of the data created
            		sigma = obsData->sigma(*w.var("x")) ;
